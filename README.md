<h1 align="center">Hi, I'm Bishesh Baruah 👋</h1>
<h3 align="center">Cybersecurity Researcher · Application & API Security · Cloud Security · BITS Pilani</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/bishesh2528"><img src="https://img.shields.io/badge/LinkedIn-bishesh2528-0A66C2?style=flat&logo=linkedin&logoColor=white"/></a>
  <a href="https://github.com/bishesh-droid"><img src="https://img.shields.io/badge/GitHub-bishesh--droid-181717?style=flat&logo=github&logoColor=white"/></a>
  <a href="mailto:bisheshbaruah56@gmail.com"><img src="https://img.shields.io/badge/Email-bisheshbaruah56@gmail.com-D14836?style=flat&logo=gmail&logoColor=white"/></a>
</p>

---

### About Me

Civil Engineering undergraduate at **BITS Pilani** (2022–2026) with a strong hands-on focus on cybersecurity and system security.
I build practical security tools across application security, cloud infrastructure, network defense, and malware analysis.

- 🔭 **80+ open-source security projects** — scanners, IDS, forensics tools, SOAR automation
- 🛡️ Focused on: **Application & API Security · Cloud Security · DFIR · Threat Intelligence**
- ☁️ Experience with **AWS misconfiguration analysis** and cloud security posture management
- 🧠 Engineering mindset centered on failure analysis, resilience, and risk reduction
- 📍 Guwahati, Assam, India

---

### Tech Stack

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Rust](https://img.shields.io/badge/Rust-000000?style=flat&logo=rust&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat&logo=gnubash&logoColor=white)

**Backend & Systems**

![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![REST APIs](https://img.shields.io/badge/REST_APIs-005571?style=flat)

**Security & Tools**

![Kali Linux](https://img.shields.io/badge/Kali_Linux-557C94?style=flat&logo=kalilinux&logoColor=white)
![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=flat&logo=wireshark&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white)
![Volatility](https://img.shields.io/badge/Volatility-Forensics-black?style=flat)

**Machine Learning**

![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat&logo=scikitlearn&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)

**Domains**

`Application & API Security` `Cloud & Infrastructure Security` `Network Security`
`Security Monitoring & Detection` `Malware & Threat Analysis` `Cryptography` `Digital Forensics` `OSINT` `SOAR`

---

### Certifications

| Certification | Issuer | Year |
|---|---|---|
| Google Cybersecurity Professional Certificate | Google | 2025 |
| CompTIA A+ | CompTIA | 2025 |
| CompTIA IT Fundamentals (ITF+) | CompTIA | 2024 |
| CompTIA Security+ | CompTIA | Expected Apr 2026 |

---

### Featured Projects

| Project | Description |
|---|---|
| [Python-Vulnerability-Scanner](https://github.com/bishesh-droid/Python-Vulnerability-Scanner) | Web scanner detecting SQLi, XSS, insecure headers, and server misconfigurations |
| [aws-cloud-scanner](https://github.com/bishesh-droid/aws-cloud-scanner) | AWS misconfiguration auditor — detects overly permissive IAM roles and exposed services |
| [idor-tester](https://github.com/bishesh-droid/idor-tester) | REST API security tool for detecting broken object-level authorization (IDOR) |
| [network-based-ids-with-ml](https://github.com/bishesh-droid/network-based-ids-with-ml) | Anomaly-based IDS using ML to detect port scans, brute-force, and lateral movement |
| [automated-malware-sandbox](https://github.com/bishesh-droid/automated-malware-sandbox) | Dynamic malware analysis sandbox using VirtualBox |
| [security_orchestration_automation_and_response_soar_playbook](https://github.com/bishesh-droid/security_orchestration_automation_and_response_soar_playbook) | Automated SOAR playbook for security incident response |
| [memory_forensics_analyzer](https://github.com/bishesh-droid/memory_forensics_analyzer) | Memory dump analysis using Volatility 3 |
| [flask-waf](https://github.com/bishesh-droid/flask-waf) | Web Application Firewall built with Python/Flask |

---

### GitHub Stats

<p align="center">
  <img src="https://streak-stats.demolab.com?user=bishesh-droid&theme=dark&hide_border=true" height="150"/>
</p>
<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=bishesh-droid&theme=react-dark&hide_border=true&area=true" />
</p>

---

### Let's Connect

I'm actively seeking roles in **Security Engineering, Application Security, Cloud Security, or SOC/DFIR**.
Feel free to reach out on [LinkedIn](https://www.linkedin.com/in/bishesh2528) or explore my work here on GitHub.
